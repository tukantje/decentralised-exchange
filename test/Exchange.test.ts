...
describe("Exchange", function () {
  ...
  it("Should be able to swap eth for tokens", async function () {
    ...
    const tokenBalanceBefore = await token.balanceOf(alice);
    const swapTx = await exchange
      .connect(alice)
      .ethToTokenSwap(hre.ethers.parseEther("0.1"), {
        value: hre.ethers.parseEther("1"),
      });
    await swapTx.wait();
    const tokenBalanceAfter = await token.balanceOf(alice);
  ...
  it("Should be able to swap tokens for eth", async function () {
    ...
    const transferTokenToAlice = await token
      .connect(owner)
      .transfer(alice, hre.ethers.parseEther("2"));
    await transferTokenToAlice.wait();
    ...
    const approvalTx = await token
      .connect(alice)
      .approve(exchange.target, hre.ethers.parseEther("2"));
    await approvalTx.wait();
    ...
    expect(await hre.ethers.provider.getBalance(exchange)).to.equal(
      hre.ethers.parseEther("1")
    );
